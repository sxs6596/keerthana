...
const PeerReview = () => {
  ...
  const [reviews, setReviews] = useState([
    {
      name: "John Doe",
      review: "Great thesis! Well-researched and detailed analysis.",
      date: "Sep 10, 2024",
    },
  ]);
  ...
  useEffect(() => {
    ...
    if (storedData && storedData.role === "Reviewer") {
      setIsReviewer(true);
    }
  }, []);
            ...
            {reviews.map((review, index) => (
              <li key={index} className="bg-neutral-focus p-4 rounded-lg">
                ...
                <div className="flex flex-row justify-end items-center gap-2">
                  <button
                    className="btn btn-sm btn-secondary border-2 text-slate-100"
                    disabled={!isReviewer}
                  >
                    Edit
                  </button>
